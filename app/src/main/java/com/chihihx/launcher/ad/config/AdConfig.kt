...
data class AdConfig(
    ...
    // 赋值方法
    fun applyConfig(config: AdConfig) {
    ...
    fun applyPluginInfo(config: PluginInfoEntity?) {
        config?.let {
            this.isClosable = config.isClosable
            this.adSkipTime = config.adSkipTime
            this.position = config.position
    ...
    fun onAdCallback(callback: AdLoadCallbackBuilder.() -> Unit): AdConfig {
...
fun buildAdCallback(config: AdConfig.() -> Unit):Pair<AdConfig, AdLoadCallbackBuilder>{
    // 创建 LambdaAdLoadCallback 实例并传递 lambda
    val adConfig = AdConfig()
    adConfig.apply(config) // 使用传入的闭包来配置参数
    val adCallback = adConfig.onAdCallback
    val callback = AdLoadCallbackBuilder().apply(adCallback)
...
enum class Position(val backendValue: Int) {
    ...
    companion object {
        fun fromInt(value: Int): Position {
            return values().firstOrNull { it.backendValue == value }
